...
@app.route('/status')
def status():
    global recording, recording_start, should_record, segment_length, led_state, red_led, power_good
    if power_good:
        if GPIO.input(red_led) == 0:
            power_good = False
    recording_duration = 0
    if recording_start is not None:
        recording_duration = time.time() - recording_start
    # Return device status as JSON
    return json.dumps({'recording': recording, 'recording_duration': recording_duration, 'should_record': should_record, 'led_state': led_state, 'temperature': str(cpu_temp()/1000), 'power_ok': power_good}), 200, {'ContentType': 'application/json'}
